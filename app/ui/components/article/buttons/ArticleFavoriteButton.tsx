...
export const ArticleFavoriteButtonWithoutContext = ({
  slug,
  favorited: originFavorited = false,
  favoritesCount: originFavoritesCount = 0,
  isSimple = false,
  className,
}: ArticleFavoriteButtonProps) => {
  const router = useRouter();
  ...
  const [favorited, setFavorited] = useState<boolean>(originFavorited);
  const [favoritesCount, setFavoritesCount] =
    useState<number>(originFavoritesCount);
